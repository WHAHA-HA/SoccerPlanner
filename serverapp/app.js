...
function informMembershipRequired(req, res, next){
  req._error_type = 'MembershipRequired';
  res.json({ok:false, error:'MembershipRequired', next:'/login'});
}

function requiredAuthentication(req, res, next) {
  //token
  //var token = req.currentUser.token;
  //ordinary mysql login
  //new wordpress authentication
  ...
  if(userkey){
    if(Config.useWPLogin){
      ...
      var wp = new WP({
        endpoint: URL ,
        username: Config.wpapi.admin,
        password: Config.wpapi.adminPassword,
        auth: true
      });
      wp.users().id(userkey).then(function(user){
        req._user = user;
        if (userkey) {
         Store.models.User.findById(userkey).then(
          function(user){
            req._user.logoUrl = user.logoUrl;
            req._user.local = user.local;
            req._user.name = user.name;
            next();
          },
          function(err){
...
function setCurrentUserToken(req, res, token){
  res.cookie('currentUser',{token: token}, {signed: true});
}
    ...
    q.then(function(data){
      ...
      if(fetchDrills){
        ...
        if(session.length > 0){
          console.log('-----------------------');
          console.log(data);
          qUser = Store.models.User.getById(session[0].owner);
          qUser.then(function(user){
            if(!user){
              failedResponse(req, res,'Author missing');
              return;
            }
            var result = _.extend(data, {author: user});
            console.log('--------------------------');
            console.log(result);
            resolve(result);
            //resolve(session[0], drills, {author: user});
          })
        ...
        qUser = Store.models.User.getById(data.owner);
        qUser.then(function(user){
          if(!user){
            failedResponse(req, res,'Author missing');
            return;
...
function processDrill(req, res, allowShareKey){
  return new Promise(function(resolve, reject){
    ...
    q.then(function(drill){
      ...
      var qUser = Store.models.User.getById(drill.owner);
      qUser.then(function(user){
        if(!user){
          failedResponse(req, res,'Author Missing');
          return;
        }
        var result = _.extend(drill, {author: user});
        resolve(result);
      })
...
app.post('/me', requiredAuthentication, fileUpload.single('file'),  function(req, res){
  ...
  function _updateUser(){
    Store.models.User.getById(req._user.id)
        .then(function(newUser){
          if(newUser !== null){
            newUser.name = user.name;
            newUser.local = user.local;
            newUser.logoUrl = user.logoUrl;
            newUser.save().then(function(){
              res.json({ok:true, user:newUser.safeData()});
...
app.get('/sessions/:sessionKey/', requiredAuthentication, function(req, res){
  processSession(req, res, true, true).then(function(data){
    console.log('**************************');
    console.log(data);
    if(data.sessions[0].safeData){
      session = data.sessions[0].safeData();
    }
    res.json({ok:true, session:session, author: data.author});
  });
});
...
app.get('/drills/:drillKey/', requiredAuthentication, function(req, res){
  processDrill(req, res, true).then(function(drill){
    res.json({ok:true, drill:drill.safeData(), author: drill.author});
  });
});
...
function handleWPLogin(req, res){
  Store.authenticateUserWPJWT(req.body.email, req.body.password).then(function(user){
    //set current user
    setCurrentUser(req, res, user);
    var membershipURL = Config.wpapi.memberPoint+user.id;
    ...
    request.get(membershipURL,
      ...
      function(err, result, body){
        ...
        //have to decide memberpress meta information
        var memberPressInfo = JSON.parse(body);
        var memberships = memberPressInfo.active_memberships;
        var premiumMemberships = ['11-25 Coaches Group Membership', '26-50 Coaches Group Membership', '5-10 Coaches Group Membership','Premium Membership','Yearly Premium Membership'];
        if(memberships){
          var isFreeMember = true;
          _.each(memberships,function(membership){
            console.log(membership);
            if(premiumMemberships.indexOf(membership.title) > -1){
              isFreeMember = false;
            }
          });

          //have proper memberhship and go next
          if(!isFreeMember && memberships.length > 0 ){
            //first check initialization of drills and sessions for basic demo content
            //populate drills and session data
            Store.initSessionsAndDrills(user.id,null,null)
                .then(function(data){
                  var next = req.query.next || '/';
                  res.json({ok:true, user:body ,next:next});
                }, function(err){
                  res.clearCookie('user');
                  res.json({ok:false, 'error_msg':+'Invalid Data Population', 'error': 'POPULATION_FAILED', next:'login/'});
                });

          }else{
            //need to upgrade membeship , redirect to login
            //first logout
            res.clearCookie('user');
            res.json({ok:false, 'error_msg':+'Invalid Membership', 'error': 'MEMBERSHIP_FAILED', next:'login/'});
          }
        }else{
          //first logout
          res.clearCookie('user');
